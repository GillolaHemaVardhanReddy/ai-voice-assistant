...
- ⚠️ **`git fetch` FIRST, before reading `progress.md` or `session-log.md` — every single session, no exceptions.** The student works across two machines. On 14 Aug 2026 (S21) I read a clone that was **4 commits behind**, so Sessions 19 and 20 were invisible, and I re-taught an atom he had already finished — including a bug he had already found himself. If `git status` says *behind*, **pull before teaching anything**. Local docs are not the source of truth; the remote is.
- `docs/progress.md` — the atom checklist + "currently at" marker. **This is the source of truth for where we are** — check it at session start (after fetching), tick atoms as completed.
